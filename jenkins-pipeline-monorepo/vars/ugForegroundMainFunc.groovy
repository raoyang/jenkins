...
import pipeline.ugfront.UgCoreDevProcess
import pipeline.ugfront.UgCoreMainProcess
import pipeline.ugfront.UgCorePreProcess
import pipeline.ugfront.UgCoreTestProcess

static def createProcess(Context context, String branchName) {
    switch (branchName) {
        case "dev":
            return new UgCoreDevProcess(context);
        case "test":
            return new UgCoreTestProcess(context);
        case "pre":
            return new UgCorePreProcess(context);
        case "main":
            return new UgCoreMainProcess(context);
    }
    return null
}
...
def call() {
    ...
    Context context = new ContextInfoCmd(this)
            .setFilePath("UgForegroundProjectConfig.yaml")

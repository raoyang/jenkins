 ...
 * 检查idMaker配置
 */
class IdMakerValidateCmd extends AbstractMiddlewareValidateCmd {

    IdMakerValidateCmd(context) {
        super(context)
    }
    ...
    AbstractMiddlewareValidateCmd execute() {
        ...
        for (def item in this.config) {
            ...
            if (ValidateUtils.ignore(this.clusterName, this.virtualEnv, item)) {
                this.context.jenkins.echo "中间件-IdMaker-忽略配置检查：item=${item.type}, cluster=${this.clusterName}, virtualEnv=${this.virtualEnv}"
                continue
            }
            ...
            def apiResult = new LateinosFrameworkApiCmd(this.context)
                    ...
                    .setApiMethod("GET")
                    .setApiPath("/v1/idmaker/info/${item.type}")
                    .execute().getResult()
            this.context.jenkins.echo "apiResult = ${apiResult}"

            // 结果解析
            if (apiResult == null || apiResult["data"] == null) {
                failedItems[item.type] = "IdMaker类型不存在"
            }
        }
